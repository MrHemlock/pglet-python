...
stack2 = Stack(
    width="70%",
    on_submit=outer_stack_submit,
    controls=[
        Text(value="Todos", size="large", align="center"),
        Stack(
            horizontal=True,
            on_submit=add_stack_submit,
            data=222,
            controls=[
                Textbox(
                    id="new_task", placeholder="Whats needs to be done?", width="100%"
                ),
        ...
        Stack(
            gap=25,
            controls=[
                Tabs(tabs=[Tab(text="all"), Tab(text="active"), Tab(text="completed")]),
                Stack(
                    horizontal=True,
