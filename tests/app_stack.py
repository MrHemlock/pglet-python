...
import pglet
from pglet import Message, Slider, Stack, Text, Textbox
...
def main(page):
    ...
    padding_slider = Slider(
        ...
        value=0,
        show_value=True,
        on_change=padding_slider_change,
    )
    ...
    page.add(
        Text("Horizontal stack - Wrapping", size="xLarge"), wrap_slider, wrap_stack
    )
    ...
    def stack_on_submit(e):
        ...
        stack.controls.insert(
            0, Message("Form has been submitted!", type="success", dismiss=True)
        )
        stack.update()
